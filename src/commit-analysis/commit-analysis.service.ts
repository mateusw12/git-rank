...
export class CommitAnalysisService {
  ...
  analyze(commits: CommitActivitySample[]): CommitAnalysisResult {
    ...
    if (totalCommitsAnalyzed === 0) {
      return {
        ...
        highlights: [
          'Nenhum commit encontrado para analise no periodo amostrado',
        ],
      };
    }
  ...
  private buildFrequencyBreakdown(
    ...
    for (const commit of commits) {
      ...
      const weekDiff = Math.floor(
        (now - authoredAt) / (7 * 24 * 60 * 60 * 1000),
      );
    ...
    const commitsPerWeek = Number(
      (totalInWindow / this.analyzedWeeks).toFixed(2),
    );
    const consistencyScore = Math.round(
      (activeWeeks / this.analyzedWeeks) * 100,
    );
  ...
  private buildScheduleBreakdown(
    commits: CommitActivitySample[],
  ): CommitScheduleBreakdown {
    ...
    const offHoursRatio = Number(
      (offHoursCommitCount / commits.length).toFixed(3),
    );
  ...
  private buildHighlights(
    ...
    if (frequency.consistencyScore >= 70) {
      ...
      highlights.push(
        'Consistencia de commits pode melhorar ao longo das semanas',
      );
    }
    ...
    if (size.averageTotalChanges >= 150) {
      highlights.push(
        'Commits grandes em media, avaliar granularidade de entrega',
      );
    } else {
